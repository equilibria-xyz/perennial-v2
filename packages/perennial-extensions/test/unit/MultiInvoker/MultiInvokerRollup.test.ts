...
import {
  ...
  VaultUpdate,
  Actions,
} from '../../helpers/rollupInvoke'

import { MAX_UINT } from '../../helpers/invoke'
...
describe('MultiInvokerRollup', () => {
  ...
  let multiInvoker: MultiInvokerRollup
  ...
  beforeEach(async () => {
    ...
    multiInvoker = await new MultiInvokerRollup__factory(owner).deploy(
      usdc.address,
      dsu.address,
  ...
  describe('#invoke', () => {
    ...
    const fixture = async () => {
      vaultUpdate = { vault: vault.address, depositAssets: 0, redeemShares: 0, claimAssets: 0 }
      ...
      await expect(
        user.sendTransaction(
          invoke(
            user,
            multiInvoker,
            await buildUpdateMarket({ market: market.address, collateral: collateral, handleWrap: false }),
          ),
        ),
      ).to.not.be.reverted
    ...
    it('withdraws collateral', async () => {
      ...
      await expect(
        user.sendTransaction(
          invoke(user, multiInvoker, buildUpdateMarket({ market: market.address, collateral: collateral.mul(-1) })),
        ),
      ).to.not.be.reverted
    ...
    it('deposits assets to vault', async () => {
      ...
      await expect(user.sendTransaction(invoke(user, multiInvoker, buildUpdateVault(vaultUpdate)))).to.not.be.reverted
    ...
    it('approves market and vault', async () => {
      ...
      let i = buildApproveTarget({ target: owner.address })

      await expect(user.sendTransaction(invoke(user, multiInvoker, i))).to.have.been.revertedWithCustomError(
        multiInvoker,
        'MultiInvokerInvalidInstanceError',
      )

      // approve market succeeds
      i = buildApproveTarget({ target: market.address })
      await expect(user.sendTransaction(invoke(user, multiInvoker, i))).to.not.be.reverted
      expect(dsu.approve).to.have.been.calledWith(market.address, constants.MaxUint256)

      // approve vault succeeds
      i = buildApproveTarget({ target: vault.address })
      await expect(user.sendTransaction(invoke(user, multiInvoker, i))).to.not.be.reverted
    ...
    it('charges an interface fee on deposit and pushes DSU from collateral to the receiver', async () => {
      ...
      await expect(user.sendTransaction(invoke(user, multiInvoker, i)))
    ...
    it('charges an interface fee on withdrawal and pushes DSU from collateral to the receiver', async () => {
      ...
      let i = buildUpdateMarket({ market: market.address, collateral: collateral })
    ...
    describe('#keeper order invoke', () => {
      ...
      it('places a limit order', async () => {
        ...
        const o = buildPlaceOrder({ market: market.address, collateral: collateral, order: trigger })
        await expect(user.sendTransaction(invoke(user, multiInvoker, o)))
      ...
      it('places a tp order', async () => {
        ...
        let o = buildPlaceOrder({ market: market.address, short: position, collateral: collateral, order: trigger })
        await expect(user.sendTransaction(invoke(user, multiInvoker, o))).to.not.be.reverted
        ...
        o = buildPlaceOrder({ market: market.address, short: position, collateral: collateral, order: trigger })

        expect(user.sendTransaction(invoke(user, multiInvoker, o))).to.not.be.reverted
      ...
      it('cancels an order', async () => {
        ...
        await expect(user.sendTransaction(invoke(user, multiInvoker, placeAction))).to.not.be.reverted

        // cancel the order
        const cancelAction = buildCancelOrder({ market: market.address, orderId: 1 })
        await expect(user.sendTransaction(invoke(user, multiInvoker, cancelAction)))
      ...
      describe('#reverts on', async () => {
        it('reverts update, vaultUpdate, placeOrder on InvalidInstanceError', async () => {
          await expect(
            user.sendTransaction(invoke(user, multiInvoker, buildUpdateMarket({ market: vault.address }))),
          ).to.be.revertedWithCustomError(multiInvoker, 'MultiInvokerInvalidInstanceError')

          await expect(
            user.sendTransaction(invoke(user, multiInvoker, buildUpdateVault({ vault: market.address }))),
          ).to.be.revertedWithCustomError(multiInvoker, 'MultiInvokerInvalidInstanceError')
        ...
        it('reverts placeOrder on InvalidOrderError', async () => {
          ...
          await expect(user.sendTransaction(invoke(user, multiInvoker, placeOrder))).to.be.revertedWithCustomError(
            multiInvoker,
      ...
      describe('#trigger orders', async () => {
        ...
        it('executes a long limit order', async () => {
          ...
          await expect(user.sendTransaction(invoke(user, multiInvoker, placeOrder))).to.not.be.reverted

          const execOrder = buildExecOrder({ user: user.address, market: market.address, orderId: 1 })
          await expect(user.sendTransaction(invoke(user, multiInvoker, execOrder)))
        ...
        it('executes a short limit order', async () => {
          ...
          await user.sendTransaction(invoke(user, multiInvoker, placeOrder))
        ...
        it('executes a maker limit order', async () => {
          ...
          user.sendTransaction(invoke(user, multiInvoker, placeOrder))
        ...
        it.only('executes an order and charges keeper fee to sender', async () => {
          ...
          await expect(owner.sendTransaction(invoke(owner, multiInvoker, execOrder, 100000000)))
            .to.emit(multiInvoker, 'OrderExecuted')
            .to.emit(multiInvoker, 'KeeperCall')
