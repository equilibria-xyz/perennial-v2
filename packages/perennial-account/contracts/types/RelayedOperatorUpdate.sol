...
library RelayedOperatorUpdateLib {
    ...
    bytes32 public constant STRUCT_HASH = keccak256(
        ...
        "AccessUpdate(address accessor,bool approved)" "Action(uint256 maxFee,Common common)"
    ...
    function hash(RelayedOperatorUpdate memory self) internal pure returns (bytes32) {
        return
            keccak256(abi.encode(STRUCT_HASH, OperatorUpdateLib.hash(self.operatorUpdate), ActionLib.hash(self.action)));
    }
}
