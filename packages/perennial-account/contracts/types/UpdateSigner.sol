...
library UpdateSignerLib {
    ...
    bytes32 constant public STRUCT_HASH = keccak256(
        "UpdateSigner(address delegate,bool newEnabled,Action action)"
    ...
    function hash(UpdateSigner memory self) internal pure returns (bytes32) {
        return keccak256(abi.encode(STRUCT_HASH, self.delegate, self.newEnabled, ActionLib.hash(self.action)));
    }
}