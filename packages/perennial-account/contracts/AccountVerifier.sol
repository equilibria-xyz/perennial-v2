...
contract AccountVerifier is VerifierBase, IAccountVerifier {
        ...
        validateAndCancel(action.common, signature)
    {
        if (
            !SignatureChecker.isValidSignatureNow(
                action.common.signer, _hashTypedDataV4(ActionLib.hash(action)), signature
            )
        ...
        validateAndCancel(deployAccount.action.common, signature)
    {
        if (
            !SignatureChecker.isValidSignatureNow(
                deployAccount.action.common.signer, _hashTypedDataV4(DeployAccountLib.hash(deployAccount)), signature
            )
        ) revert VerifierInvalidSignerError();
    }
        ...
        validateAndCancel(marketTransfer.action.common, signature)
    {
        if (
            !SignatureChecker.isValidSignatureNow(
                marketTransfer.action.common.signer, _hashTypedDataV4(MarketTransferLib.hash(marketTransfer)), signature
            )
        ) revert VerifierInvalidSignerError();
    }
        ...
        validateAndCancel(change.action.common, signature)
    {
        if (
            !SignatureChecker.isValidSignatureNow(
                change.action.common.signer, _hashTypedDataV4(RebalanceConfigChangeLib.hash(change)), signature
            )
        ) revert VerifierInvalidSignerError();
    }
        ...
        validateAndCancel(withdrawal.action.common, signature)
    {
        if (
            !SignatureChecker.isValidSignatureNow(
                withdrawal.action.common.signer, _hashTypedDataV4(WithdrawalLib.hash(withdrawal)), signature
            )
        ) revert VerifierInvalidSignerError();
    }

    /// @inheritdoc IRelayVerifier
    function verifyRelayedNonceCancellation(RelayedNonceCancellation calldata message, bytes calldata outerSignature)
        external
        validateAndCancel(message.action.common, outerSignature)
    {
        if (
            !SignatureChecker.isValidSignatureNow(
    ...
    /// @inheritdoc IRelayVerifier
    function verifyRelayedGroupCancellation(RelayedGroupCancellation calldata message, bytes calldata outerSignature)
        external
        validateAndCancel(message.action.common, outerSignature)
    {
        if (
    ...
    /// @inheritdoc IRelayVerifier
    function verifyRelayedOperatorUpdate(RelayedOperatorUpdate calldata message, bytes calldata outerSignature)
        external
        validateAndCancel(message.action.common, outerSignature)
    {
        if (
            !SignatureChecker.isValidSignatureNow(
                message.action.common.signer, _hashTypedDataV4(RelayedOperatorUpdateLib.hash(message)), outerSignature
            )
        ) revert VerifierInvalidSignerError();
    }

    /// @inheritdoc IRelayVerifier
    function verifyRelayedSignerUpdate(RelayedSignerUpdate calldata message, bytes calldata outerSignature)
        external
        validateAndCancel(message.action.common, outerSignature)
    {
        if (
            !SignatureChecker.isValidSignatureNow(
                message.action.common.signer, _hashTypedDataV4(RelayedSignerUpdateLib.hash(message)), outerSignature
            )
        ) revert VerifierInvalidSignerError();
    }

    /// @inheritdoc IRelayVerifier
    function verifyRelayedAccessUpdateBatch(RelayedAccessUpdateBatch calldata message, bytes calldata outerSignature)
        external
        validateAndCancel(message.action.common, outerSignature)
    {
        if (
