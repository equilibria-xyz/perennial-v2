  ...
  describe('#deposit/#redeem/#claim/#settle', () => {
    ...
    context('liquidation', () => {
      context('long', () => {
        it('recovers from a liquidation', async () => {
          ...
          await btcMarket.connect(user).update(vault.address, 0, 0, 0, EXPECTED_LIQUIDATION_FEE.mul(-1))
    ...
    context('insolvency', () => {
      it('gracefully unwinds upon totalClaimable insolvency', async () => {
        ...
        await market.connect(user).update(vault.address, 0, 0, 0, EXPECTED_LIQUIDATION_FEE.mul(-1))
