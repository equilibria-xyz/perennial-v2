...
contract VaultFactory is IVaultFactory, Factory {
    ...
    constructor(IMarketFactory marketFactory_, address implementation_, UFixed6 initialAmount_)
    ...
    function create(Token18 asset, IMarket initialMarket, string calldata name)
        external
        onlyOwner
        returns (IVault newVault)
    {
        // create vault
        newVault =
            IVault(address(_create(abi.encodeCall(IVault.initialize, (asset, initialMarket, initialAmount, name)))));
