...
// This VAA has timestamp 1686198989.
const OTHER_VAA =
...
const STARTING_TIME = 1686198972
...
describe('PythOracle', () => {
  ...
  beforeEach(async () => {
    ...
    await time.increaseTo(STARTING_TIME)
    // block.timestamp of the next call will be 1686198973 (STARTING_TIME + 1)
  })
  ...
  describe('#atVersion', async () => {
    it('returns the correct version', async () => {
      ...
      const version = await oracle.connect(user).at(STARTING_TIME + 1)
      expect(version.valid).to.be.true
      expect(version.price).to.equal('18381670317700000000000000')
    })
