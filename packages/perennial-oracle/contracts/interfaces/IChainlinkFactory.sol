...
interface IChainlinkFactory is IOracleProviderFactory, IFactory {
    struct Granularity {
    ...
    // sig: 0x3d225882
    error ChainlinkFactoryNotInstanceError();
    // sig: 0xa7cc0264
    error ChainlinkFactoryInvalidGranularityError();
    // sig: 0xf2f2ce54
    error ChainlinkFactoryAlreadyCreatedError();

    function initialize(IOracleFactory oracleFactory) external;
    function create(bytes32 id) external returns (IChainlinkOracle oracle);
