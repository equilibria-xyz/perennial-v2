...
contract ChainlinkFactory is IChainlinkFactory, Factory {
    ...
    /// @param implementation_ IChainlinkOracle implementation contract
    ...
    function create(bytes32 id) external onlyOwner returns (IChainlinkOracle newOracle) {
        if (oracles[id] != IOracleProvider(address(0))) revert ChainlinkFactoryAlreadyCreatedError();

        newOracle = IChainlinkOracle(address(
            _create(abi.encodeCall(IChainlinkOracle.initialize, (id, ethTokenChainlinkFeed, keeperToken)))));
    ...
    function updateGranularity(uint256 newGranularity) external onlyOwner {
        ...
        if (newGranularity == 0) revert ChainlinkFactoryInvalidGranularityError();
        if (_current <= uint256(_granularity.effectiveAfter)) revert ChainlinkFactoryInvalidGranularityError();
        if (newGranularity > MAX_GRANULARITY) revert ChainlinkFactoryInvalidGranularityError();
