...
contract PythOracle is IPythOracle, Instance, Kept {
        ...
        payable
    {
        ...
        UFixed18 keeperFee = UFixed18Lib.from(gasUsed)
            .mul(KEEPER_REWARD_PREMIUM)
            .add(UFixed18Lib.from(KEEPER_BUFFER))
            .mul(etherPrice())
            .mul(UFixed18.wrap(block.basefee));

        _raiseKeeperFee(keeperFee, "");

        keeperToken().push(msg.sender, keeperFee);

        emit KeeperCall(msg.sender, gasUsed, KEEPER_REWARD_PREMIUM, KEEPER_BUFFER, keeperFee);
    }

    function etherPrice() private view returns (UFixed18) {
