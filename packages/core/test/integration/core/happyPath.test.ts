...
describe('Happy Path', () => {
  ...
  it('resolve market shortfall from insurance fund', async () => {
    const POSITION = parse6decimal('10')
    const POSITION_B = parse6decimal('1')
    const COLLATERAL = parse6decimal('1000')
    const { owner, user, userB, dsu, chainlink, marketFactory, insuranceFund } = instanceVars

    const market = await createMarket(instanceVars)
    await dsu.connect(user).approve(market.address, COLLATERAL.mul(1e12))
    await dsu.connect(userB).approve(market.address, COLLATERAL.mul(1e12))

    // Enable insurance fund as operator for market factory owner
    ...
    // resolve market shortfall
    await insuranceFund.connect(owner).resolve(market.address)
