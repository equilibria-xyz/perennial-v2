...
describe('Liquidate', () => {
  ...
  it('liquidates a user with maxLiquidationFee set to 5', async () => {
    const POSITION = parse6decimal('10')
    const COLLATERAL = parse6decimal('1000')
    const { user, userB, dsu, chainlink, owner } = instanceVars
    ...
    expect((await market.locals(userB.address)).claimable).to.equal(parse6decimal('5'))
    await market.connect(userB).claimFee(userB.address) // liquidator withdrawal

    expect(await dsu.balanceOf(userB.address)).to.equal(utils.parseEther('200005')) // Original 200000 + fee
    expect((await market.locals(user.address)).collateral).to.equal(parse6decimal('1000').sub(parse6decimal('6')))
    expect(await dsu.balanceOf(market.address)).to.equal(utils.parseEther('1000').sub(utils.parseEther('5')))
