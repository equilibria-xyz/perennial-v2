...
library IntentLib {
    ...
    function hash(Intent memory self) internal pure returns (bytes32) {
        return keccak256(abi.encode(STRUCT_HASH, self.amount, self.collateral, self.price, self.fee, self.additiveFee, self.originator, self.solver, CommonLib.hash(self.common)));
    }
}
