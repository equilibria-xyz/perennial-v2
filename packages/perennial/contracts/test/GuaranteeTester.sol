...
abstract contract GuaranteeTester {
    function read() public view virtual returns (Guarantee memory);

    function store(Guarantee memory newGuarantee) public virtual;

    function from(Order memory order, Fixed6 price, UFixed6 referralFee, bool chargeSettlementFee, bool chargeTradeFee)
        external
    {
        Guarantee memory newGuarantee =
            GuaranteeLib.from(order, price, referralFee, chargeSettlementFee, chargeTradeFee);
        store(newGuarantee);
    }
